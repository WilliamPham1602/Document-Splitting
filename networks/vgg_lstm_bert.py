...
class Vgg_Lstm_Bert(nn.Module):

    def __init__(self, img_dim=3, embedding_dim=256, hidden_dim=128, output_dim=2, n_layers=2, bidirectional=True, dropout=0.2):
        super(Vgg_Lstm_Bert, self).__init__()
    ...
    def forward(self, x1, text_seq):
        ...
        x = torch.cat((x1, x2), 1)
        #
        dense_outputs = self.combination(x)
...
if __name__ == '__main__':
    ...
    print(data.shape)
    model = Vgg_Lstm_Bert(img_dim=3, embedding_dim=256, output_dim=2)
